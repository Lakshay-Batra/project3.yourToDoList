...
app.post("/register", (req, res) => {
    ...
    User.register({ username: username, firstName: firstName, lastName: lastName }, password, (err, user) => {
        if (err) {
            console.log(err);
            res.redirect("/register");
        } else {
            passport.authenticate("local", { failureRedirect: "/register", failureFlash: true })(req, res, () => {
                res.redirect("/");
            });
        }
    });
            ...
            if (!err) {
                if (foundUser) {
                    if (foundUser.items.length == 0) {
                        User.findOneAndUpdate({ _id: req.user._id }, { $set: { items: defaultItems } }, (err) => {
                            if (!err) {
                                res.redirect("/");
                            }
                        });
                    } else {
                        res.render("list", { listTitle: day, newItems: foundUser.items, name: req.user.firstName, listNames: foundUser.listNames });
                    }
                }
            }
        })
...
app.post("/", (req, res) => {
    if (req.isAuthenticated()) {
        ...
        if (listTitle === date.getDate()) {
            User.findOneAndUpdate({ _id: req.user._id }, { $push: { items: newitem } }, (err) => {
                if (!err) {
                    res.redirect("/");
                }
            });
        } else {
            User.findOneAndUpdate({ _id: req.user._id, lists: { $elemMatch: { name: listTitle } } }, { $push: { "lists.$.items": newitem } }, (err) => {
                if (!err) {
...
app.post("/delete", (req, res) => {
    if (req.isAuthenticated()) {
        ...
        if (listName === date.getDate()) {
            User.findOneAndUpdate({ _id: req.user._id }, { $pull: { items: { _id: id } } }, (err) => {
                if (!err) {
                    res.redirect("/");
                }
            });
        } else {
            User.findOneAndUpdate({ _id: req.user._id, lists: { $elemMatch: { name: listName } } }, { $pull: { "lists.$.items": { _id: id } } }, (err) => {
                if (!err) {
...
app.get("/lists/:listName", (req, res) => {
    if (req.isAuthenticated()) {
        ...
        User.findOne({ _id: req.user._id }, { lists: { $elemMatch: { name: customListName } } }, (err, foundList) => {
            if (foundList.lists[0].items.length == 0) {
                ...
                User.findOne({ _id: req.user._id }, (err, foundUser) => {
                    if (!err) {
                        if (foundUser) {
                            res.render("list", { listTitle: foundList.lists[0].name, newItems: foundList.lists[0].items, name: req.user.firstName, listNames: foundUser.listNames });
                        }
                    }
                });
...
app.get("/deleteList/:listName", (req, res) => {
    if (req.isAuthenticated()) {
        ...
        User.findOneAndUpdate({ _id: req.user._id }, { $pull: { listNames: listName, lists: { name: listName } } }, (err) => {
            if (!err) {
                res.redirect("/");
            }
        });
